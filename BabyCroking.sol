    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address public immutable CRK = address(0x16a43134EB6b6a9438308911a133b4022B6C428e); //CAKE
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function _transfer(
        ...
        require(false, "BABYCROKING_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "BABYCROKING_Dividend_Tracker: withdrawDividend disabled. Use the 'claim'"
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BABYCROKING_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        uint256 _oldClaimWait = claimWait;
        require(
            newClaimWait != _oldClaimWait,
            "BABYCROKING_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract BabyCrokingToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable CRK = address(0x16a43134EB6b6a9438308911a133b4022B6C428e); // CRK
    ...
    uint256 public crkRewardsFee = 5;
    uint256 public liquidityFee = 2;
    uint256 public marketingFee = 5;
    uint256 public totalFees = crkRewardsFee.add(liquidityFee).add(marketingFee);
    ...
    function setFees(
        uint256 _crkRewardsFee,
        uint256 _liquidityFee,
        uint256 _marketingFee
    ) external onlyOwner {
        require(
            _crkRewardsFee + _liquidityFee + _marketingFee <= 20,
            "BCK: Too high fees"
        );
        ...
        totalFees = _crkRewardsFee.add(_liquidityFee).add(_marketingFee);

        emit FeesChanged(_crkRewardsFee, _liquidityFee, _marketingFee);
    }
    ...
    function swapAndSendToFee(uint256 tokens, address dest) private {
        ...
        super._transfer(address(this), dest, tokens);
